 ...
 */
public class WallKicker implements Player {
    ...
    public void kickWall() {
        ...
        Vector jumpVector = getVelocity().clone();
        jumpVector.multiply(3);
        jumpVector.setY(0.7);
        if (jumpVector.getX() != 0 || jumpVector.getZ() != 0) {
            Vector oppositeFace = getFacing().getOppositeFace().getDirection();
            if (oppositeFace.getX() == 0) {
                jumpVector.setZ(oppositeFace.getZ() * 0.3);
            }
            if (oppositeFace.getZ() == 0) {
                jumpVector.setX(oppositeFace.getX() * 0.3);
            }
        } else {
            if (!hasPermission(VERTICALKICK_PERMISSION)) return;
        }
     ...
     * <ul>
     * <li>Player must has permission to wall kick.</li>
     * <li>Player must be Jumping.</li>
     * <li>Wall must exist in the direction the player is facing.</li>
    ...
    /**
     * Returns true if the player has the permission to kick wall.
     *
     * @return True if player is wallkicker.
     */
    public boolean isWallKicker() {
        return hasPermission(WALLKICKER_PERMISSION);
    }
    ...
    public boolean isVerticalKicker() {
        return hasPermission(VERTICALKICK_PERMISSION);
    }
    ...
    /**
     * Returns true if the distance to the wall is less than or equal to footReach.
     * @return True if the player's feet can reach the wall.
     */
    public boolean reachWall() {
        RayTraceResult result = getWorld().rayTraceBlocks(getLocation(),getFacing().getDirection(),1);
        if (result == null) return false;
        return result.getHitPosition().distance(getLocation().toVector()) <= footReach;
    }
    ...
    /**
     * Sets the number of kick combo of the player.
     * @param kickCombo the number of kick combo.
     */
    ...
    //Delegation of the Player method below.
